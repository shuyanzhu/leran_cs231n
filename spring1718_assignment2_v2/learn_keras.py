...
x = tf.layers.Dropout()(x)
predictions = tf.layers.Dense(10, activation='relu', kernel_initializer=initializer)(x)

model  = tf.keras.Model(inputs=inputs, outputs=predictions)
model.compile(optimizer=tf.train.MomentumOptimizer(2e-2, 0.9, use_nesterov=True),
              loss=tf.keras.losses.sparse_categorical_crossentropy,
              metrics=['accuracy'])

# 训练模型
model.fit(X_train, y_train, batch_size=640, epochs=10, validation_data=(X_val, y_val))